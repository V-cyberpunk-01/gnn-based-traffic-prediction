...
class ChebConv(nn.Module):
    ...
    def cheb_polynomial(self, laplacian):
        ...
        multi_order_laplacian = torch.zeros([self.K, N, N], device=laplacian.device, dtype=torch.float)  # [K, N, N] k 阶数
...
class ChebNet(nn.Module):
    def __init__(self, in_c, hid_c, out_c, K):
        """
        :param in_c: int, number of input dim
        :param hid_c: int, number of hidden dim
        :param out_c: int, number of output dim
        :param K: int, 切比雪夫的展开项
        """
    ...
    def forward(self, data, device):
        ...
        flow_x = data["flow_x"].to(device)  # [B, N, H, D] bench_size, node, history, flow_x dim

        B, N = flow_x.size(0), flow_x.size(1)

        flow_x = flow_x.view(B, N, -1)  # [B, N, H*D] 从4维变成3维

        output_1 = self.act(self.conv1(flow_x, graph_data))  #act非线性激活函数
        output_2 = self.act(self.conv2(output_1, graph_data))

        return output_2.unsqueeze(2)    #维度扩张
